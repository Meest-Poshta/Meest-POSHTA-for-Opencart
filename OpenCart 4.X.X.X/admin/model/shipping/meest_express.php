...
class MeestExpress extends Model
{
    ...
    private function createTables()
    {
        ...
        $column = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "meest_express_order_shipping_data` LIKE 'customer_pay';");
        if (!$column->num_rows) {
            $this->db->query("ALTER TABLE `" . DB_PREFIX . "meest_express_order_shipping_data` ADD `customer_pay` TINYINT DEFAULT 0 AFTER `address_code`;");
        }
