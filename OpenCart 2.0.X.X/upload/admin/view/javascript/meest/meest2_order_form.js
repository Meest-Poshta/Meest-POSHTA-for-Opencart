    ...
    function populateRecipientAddresses(city_id) {
        $.ajax({
            ...
            success: function (data) {
                ...
                var currentValue = addressSelect.val();
                var currentText = addressSelect.find('option:selected').text();

                addressSelect.empty();
                addressSelect.append('<option value="">' + config.text_select + '</option>');

                // Якщо є поточне значення (наприклад, текстова адреса), зберігаємо його
                if (currentValue && currentValue !== '') {
                    addressSelect.append('<option value="' + currentValue + '" selected>' + (currentText || currentValue) + '</option>');
                }

                $.each(data, function (index, address) {
                    var addressValue = address.street_id;
                    var addressText = address.type_ua + ' ' + address.name_ua;
                    // Не додаємо дублікат якщо вже є поточне значення
                    if (!currentValue || addressValue !== currentValue) {
                        addressSelect.append('<option value="' + addressValue + '">' + addressText + '</option>');
                    }
                });

                // Реініціалізуємо select2 з можливістю вводу тексту вручну
                addressSelect.select2({
                    placeholder: config.text_select,
                    allowClear: true,
                    tags: true,  // Дозволяємо вводити текст вручну
                ...
                // Відновлюємо поточне значення
                if (currentValue) {
                    addressSelect.val(currentValue).trigger('change');
                }
            }
        });
    }
    ...
    if (config.order_shipping_data && config.order_shipping_data.city_code) {
        ...
        if (shippingMethod === 'meest2.warehouse' || shippingMethod === 'meest2.postomat' || shippingMethod === 'meest2.branch') {
            ...
            if (addressCode) {
                ...
                var $addressSelect = $('#input-recipient-address');
                var addressOption = new Option(addressName, addressCode, true, true);
