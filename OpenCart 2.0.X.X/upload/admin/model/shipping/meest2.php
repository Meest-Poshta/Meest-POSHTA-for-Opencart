...
class ModelShippingMeest2 extends Model {
    ...
    public function install($typeInstall = true) {
        ...
        if ($current_version === null || $typeInstall) {
            ...
            // Додаємо колонку для зберігання назви вулиці
            $column = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "meest2_order_shipping_data` LIKE 'address_name';");
            if (!$column->num_rows) {
                $this->db->query("ALTER TABLE `" . DB_PREFIX . "meest2_order_shipping_data` ADD `address_name` VARCHAR(255) DEFAULT NULL AFTER `address_code`;");
            }
...
//    }
    ...
    public function searchCities($search = '') {
        $sql = "SELECT c.city_id, c.name_ua, r.region_name_ua 
                FROM " . DB_PREFIX . "meest2_cities c
                LEFT JOIN " . DB_PREFIX . "meest2_regions r ON c.region_id = r.region_id";
        
        if (!empty($search)) {
            $sql .= " WHERE c.name_ua LIKE '%" . $this->db->escape($search) . "%'";
        }
        
        $sql .= " ORDER BY c.name_ua LIMIT 50";
...
//            'branch_type_app',
    ...
    public function updateOrderShippingData($order_id, $data) {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "meest2_order_shipping_data` SET
            ...
            address_name = '" . $this->db->escape(isset($data['address_name']) ? $data['address_name'] : '') . "',
